...
public class TimeLineModeActivity extends GetTwitterActivity implements
	...
	private void requestForTwitters(final MyRequestState requestState) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				case VIEW_BILATERAL:
					if (bilateralFeature == FEATURE.ALL
				...
				case VIEW_AT_ME:
					if (atMeFilter == AUTHOR_FILTER.ALL
							&& requestState.isRefresh) {
						requestState.dataNeedSave = true;
					}
				...
				case VIEW_COMMENT:
					if (commentFilter == AUTHOR_FILTER.ALL
							&& requestState.isRefresh) {
						requestState.dataNeedSave = true;
					}
	...
	private void clearState(final int type) {
		UnReadAPI unReadAPI = new UnReadAPI(MagicApplication.getInstance()
				.getAccessToken());
